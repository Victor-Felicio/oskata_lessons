# Sistema Interativo Multifuncional em Python (S.I.M.P.).

import random

ARQUIVO_NOMES = "nomes.txt"

def salvar_dados():
    with open(ARQUIVO_NOMES, "w", encoding="utf-8") as arquivo:
        for nome in lista_nomes:
            arquivo.write(nome+"\n")

def carregar_dados():
    global lista_nomes
    try:
        with open(ARQUIVO_NOMES, "r", encoding="utf-8") as arquivo:
            lista_nomes = [linha.strip() for linha in arquivo if linha.strip()]
    except FileNotFoundError:   # o que são 'buildins'?
        lista_nomes = []

def ler_inteiro(mensagem):
    while True:
        try:
            return int(input(mensagem))
        except ValueError:
            print("Erro: digite um número inteiro válido.")


def ler_float(mensagem):
    while True:
        try:
            return float(input(mensagem))
        except ValueError:
            print("Erro: digite um número válido.")


def ler_lista_numeros():
    while True:
        entrada = input("Digite números separados por espaço: ")
        try:
            return list(map(float, entrada.split()))
        except ValueError:
            print("Erro: digite apenas números.")


def ler_opcao(opcoes_validas):
    while True:
        opcao = input("Escolha: ")
        if opcao in opcoes_validas:
            return opcao
        else:
            print("Opção inválida!")

# Lista para manipulação de dados.

lista_nomes = []

# FUNÇÕES MATEMÁTICAS.
def somar():
    a = ler_float("Digite o primeiro número: ")
    b = ler_float("Digite o segundo número: ")
    print("Resultado:", a + b)

def par_ou_impar():
    n = ler_inteiro("Digite um número: ")
    if n % 2 == 0:
        print("Número par.")
    else:
        print("Número ímpar.")

def media():
    lista = ler_lista_numeros()

    if not lista:
        print("Lista vazia!")
        return

    print("Média:", sum(lista) / len(lista))

def fatorial():
    while True:
        n = ler_inteiro("Digite um número NÃO negativo (máx. 100): ")

        if n < 0:
            print("Não existe fatorial de número negativo.")
        elif n > 100:
            print("Erro! Número acima do valor máximo (100)!")
        else:
            break

    resultado = 1
    for i in range(1, n + 1):
        resultado *= i

    print("Fatorial:", resultado)

def primo():
    n = ler_inteiro("Digite um número: ")

    if n < 2:
        print("Não é primo.")
        return

    for i in range(2, int(n ** 0.5) + 1):
        if n % i == 0:
            print("Não é primo.")
            return

    print("É primo.")

def maior_menor():
    lista = ler_lista_numeros()

    if not lista:
        print("Lista vazia!")
        return

    print("Maior:", max(lista))
    print("Menor:", min(lista))

# TEXTO.
def contar_vogais():
    texto = input("Digite uma palavra: ").lower()
    vogais = "aeiou"
    contador = sum(1 for letra in texto if letra in vogais)
    print("Quantidade de vogais:", contador)


def inverter_texto():
    texto = input("Digite um texto: ")
    print("Invertido:", texto[::-1])


def contar_palavras():
    frase = input("Digite uma frase: ")
    print("Quantidade de palavras:", len(frase.split()))


# DADOS.
def adicionar_nomes():
    while True:
        nome = input("Digite um nome (ou 'sair' para parar): ")
        if nome.lower() == 'sair':
            break
        lista_nomes.append(nome)
    salvar_dados()  # DÚVIDA SOBRE A IDENTAÇÃO
    print("Lista atual: ")
    for nome in lista_nomes:
        print("-", nome)
    print("Nomes adicionados com sucesso!")

def remover_nome():
    if not lista_nomes:
        print("A lista está vazia.")
        return
    print("Lista atual:")
    for nome in lista_nomes:
        print("-", nome)
    entrada = input("Digite os nomes que deseja remover separados por vírgula: ")
    nomes_remover = [nome.strip() for nome in entrada.split(",")]

    removidos = []
    nao_encontrados = []

    for nome in set(nomes_remover):
        if nome in lista_nomes:
            lista_nomes.remove(nome)
            removidos.append(nome)
        else:
            nao_encontrados.append(nome)

    if removidos:
        salvar_dados()
        print("Removidos: ", removidos)

    if nao_encontrados:
        print("Não encontrados: ", nao_encontrados)

    print("Lista atual:")
    for nome in lista_nomes:
        print("-", nome)

def exibir_nomes():
    if not lista_nomes:
        print("Lista vazia.")
    else:
        print("Lista de nomes:")
        for nome in lista_nomes:
            print("-", nome)

# EXTRAS.
def numero_aleatorio():
    print("Número aleatório: ", random.randint(1, 100))


def login_simples():
    usuario_correto = "Victor"
    senha_correta = "8118"

    usuario = input("Usuário: ")
    senha = input("Senha: ")

    if usuario == usuario_correto and senha == senha_correta:
        print("Login bem-sucedido!")
    else:
        print("Usuário ou senha incorretos.")

#  MENUS.

def menu_matematica():
    while True:
        print("=== MENU MATEMÁTICA ===")
        print("1 - Somar dois números")
        print("2 - Par ou ímpar")
        print("3 - Média de números")
        print("4 - Fatorial")
        print("5 - Número primo")
        print("6 - Maior e menor número")
        print("0 - Voltar")

        opcao = ler_opcao(["1", "2", "3", "4", "5", "6", "0"])

        if opcao == "1":
            somar()
        elif opcao == "2":
            par_ou_impar()
        elif opcao == "3":
            media()
        elif opcao == "4":
            fatorial()
        elif opcao == "5":
            primo()
        elif opcao == "6":
            maior_menor()
        elif opcao == "0":
                salvar_dados()
                print("Dados salvos!")
                break

def menu_texto():
    while True:
        print("=== MENU TEXTO ===")
        print("1 - Contar vogais.")
        print("2 - Inverter texto.")
        print("3 - Contar palavras.")
        print("0 - Voltar.")

        opcao = ler_opcao(["1", "2", "3", "0"])

        if opcao == "1":
            contar_vogais()
        elif opcao == "2":
            inverter_texto()
        elif opcao == "3":
            contar_palavras()
        elif opcao == "0":
            salvar_dados()
            print("Dados salvos!")
            break
        else:
            print("Opção inválida!")


def menu_dados():
    while True:
        print("=== NOMES ===")
        print("1 - Adicionar nomes.")
        print("2 - Remover nome.")
        print("3 - Exibir lista.")
        print("0 - Voltar.")

        opcao = ler_opcao(["1", "2", "3", "0"])

        if opcao == "1":
            adicionar_nomes()
        elif opcao == "2":
            remover_nome()
        elif opcao == "3":
            exibir_nomes()
        elif opcao == "0":
            salvar_dados()
            print("Dados salvos!")
            break
        else:
            print("Opção inválida!")

def menu_extras():
    while True:
        print("=== EXTRAS ===")
        print("1 - Criar número aleatório de 1 a 100.")
        print("2 - Login.")
        print("0 - Voltar.")

        opcao = ler_opcao(["1", "2", "0"])

        if opcao == "1":
            numero_aleatorio()
        elif opcao == "2":
            login_simples()
        elif opcao == "0":
            break
        else:
            print("Opção inválida!")

def main():
    while True:
        print("=== OPÇÕES: ===")
        print("1 - Operações Matemáticas.")
        print("2 - Manipulação de Texto.")
        print("3 - Lista de Nomes.")
        print("4 - Extras.")
        print("0 - Encerrar.")

        opcao = ler_opcao(["1", "2", "3", "4", "0"])

        if opcao == "1":
            menu_matematica()
        elif opcao == "2":
            menu_texto()
        elif opcao == "3":
            menu_dados()
        elif opcao == "4":
            menu_extras()
        elif opcao == "0":
            while True:
                confirmar = input("Certeza que deseja encerrar? [s/n]: ").strip().lower()
                if confirmar == "s":
                    salvar_dados()
                    print("Dados salvos. Até mais!")
                    return
                elif confirmar == "n":
                    print("Continuando...")
                    break
                else:
                    print("Digite APENAS 's' ou 'n'.")

carregar_dados()
main()